...
class Reader:
    ...
    def __init__(
        ...
        ephys_name = "ephys"
        protocol_names = ["contra_random_squares"]
        ...
        if os.path.isdir(path):
            ...
            #### find out if there is a blank in the stimulus via visprot and specify below
            # Read sync channel from NI-DAQ.
            self.sync_channel = SyncChannel(nidaq_metadata, nidaq_binary)
            # Read other digital channels.
    ...
    def _parse_SGLX_dir(
        ...
        for topdir, _, files in os.walk(SGLX_path):
            if topdir == SGLX_path:
                if (len(files) == 2) & files[0].endswith(".nidq.bin") & files[1].endswith(".nidq.meta"):
                    nidaq_path = topdir
                else:
                    nidaq_path = None
